#!/usr/bin/env python3
"""Share Link Saver | 分享链接保存工具 | 纯 Python · 跨平台 · 零依赖

用法:
  python3 share-link-saver.py <分享链接>
  python3 share-link-saver.py <分享链接> --out ./output
  python3 share-link-saver.py <分享链接> --format txt --json
"""
...
def to_txt(result: dict) -> str:
    sep = "=" * 60
    lines = [f"分享对话 — {result['title']}", f"来源: {result['url']}", f"模型: {result['model']}", f"导出: {datetime.now():%Y-%m-%d %H:%M}", "", sep, ""]
    for t in result["texts"]:
...
def main():
    ...
    p = ap.ArgumentParser(description="Share Link Saver | 分享链接保存工具 (纯 Python · 跨平台)")
    ...
    base = os.path.join(args.out, f"chat_{slug}_{sid}")
    if args.format == "md":
        path = base + ".md"
